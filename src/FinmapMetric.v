...
move => t s IH X. specialize (IH (fun x A => X x (mem_cons t A))). case: IH => [m IH].
case: (X t (mem_head t _)) => [m' Pm']. exists (maxn m m'). move => x I.
rewrite in_cons in I. case_eq (x == t) => E.
- rewrite (eqP E). apply: (A _ _ _ _ Pm'). by rewrite leq_maxr.
- rewrite E in I. simpl in I. specialize (IH _ I). apply: (A _ _ _ _ IH). by rewrite leq_maxl.
Qed.
