...
- have En:=Mrel_mono L E'. rewrite (minn_idPl L). by apply (Mrel_trans E En).
- rewrite leqNgt in L. have L':=negbFE L. clear L. have L:=ltnW L'. clear L'.
  rewrite (minn_idPr L). by apply (Mrel_trans (Mrel_mono L E) E').
Qed.
...
rewrite leq_maxl in X. rewrite -> leq_maxr in X'.
rewrite <- X ; last by []. by rewrite <- X'.
Qed.
...
have A:x <= maxn x x' by rewrite -> leq_maxl.
specialize (X (leq_add (leq0n n) A)).
rewrite -> leq_maxr in X'.
...
Module FMet. Section fmet.
  Variable M1 M2 : metricType.
  Record mixin_of (f:M1 -> M2) := Mixin { nonexp :> nonexpansive f }.
  Notation class_of := mixin_of (only parsing). (*CLEAR*) 
  Lemma ner (f:M1 -> M2) (ne:nonexpansive f) : setoid_respect f.
move => x x' e.
apply (proj1 (Mrefl _ _)) => n. apply ne. by apply (proj2 (Mrefl _ _)).
Qed.

  Section ClassDef.
  Definition base2 (f:M1 -> M2) (c:class_of f) : FSet.mixin_of f := fsetMixin (ner c). (*CLEARED*)
  Local Coercion base2 : class_of >-> FSet.mixin_of.
  Structure type : Type := Pack {sort : M1 -> M2; _ : class_of sort; _ : M1 -> M2}. (*CLEAR*)
  ...
  Definition fset f : fset M1 M2 := FSet.Pack (class f) f.
  End ClassDef.
 (*CLEARED*) End fmet.
Module Import Exports.
...
specialize (B (maxn m m')). rewrite -> leq_maxr in B.
rewrite -> leq_maxl in A.
...
simpl in C. simpl in C'.
rewrite geq_max in L. specialize (C (proj1 (andP L))). specialize (C' (proj2 (andP L))).
...
specialize (X(maxn n m0)). rewrite leq_maxr in X.
...
simpl. rewrite addnC in B. rewrite leq_maxl in B.
by apply: (Mrel_trans (B _)).
Qed.
...
rewrite <- X. rewrite <- Y. apply: (bounded_contractive_n C x y). by rewrite leq_maxl.
- rewrite leq_max. rewrite leq_maxr. by rewrite orbT.
- rewrite leq_max. rewrite leq_maxl. by rewrite orbT.
Qed.
...
specialize (X (maxn m m')). specialize (Y (maxn m m')).
rewrite <- X ; last by rewrite leq_maxl.
rewrite <- Y ; last by rewrite leq_maxr.
...
rewrite <- A ; last by rewrite leq_maxl.
rewrite <- B ; last by rewrite leq_maxr.
clear A. simpl. by apply fnonexpansive.
Qed.
...
specialize (A (maxn n m0)). specialize (X (maxn n m0)).
rewrite <- X ; last by rewrite leq_maxr.
by rewrite -> (fun B => (A B x)) ; last by rewrite leq_maxl.
Qed.
...
specialize (X (maxn n m)).
rewrite <- X ; last by rewrite leq_maxr.
have C:=cchain_cauchy c. specialize (C n j (maxn n m) L).
by rewrite <- (fun X => C X i) ; last by rewrite leq_maxl.
Qed.
