...
Lemma upred_prod_down (X:(upred_metricType (cvalue O) * upred_metricType (cvalue O))) :
   downclosed (fun kt => let (v,p) := snd kt:(cvalue O) in
                             match v as v0 return closedV v0 -> Prop with 
                               | P v0 v1 =>
                                    fun D => (fst X) (fst kt, CValue v0 (proj1 (andP D))) /\
                                             (snd X) (fst kt, CValue v1 (proj2 (andP D)))
                               | _ => fun _ => False end p).
...
Definition upred_prod (p:((upred_cmetricType (cvalue O)) * (upred_cmetricType (cvalue O))) %CAT)
  : upred_cmetricType (cvalue O) := Eval hnf in mk_upred (@upred_prod_down p).
...
Lemma upred_sumP (X:upred_cmetricType (cvalue O) * upred_cmetricType (cvalue O)) :
  downclosed (fun kt => let (v,p) := snd kt:cvalue O in 
                             match v as v0 return closedV v0 -> Prop with
                               | (INL _ p0) => fun P => (fst X) (fst kt, CValue p0 P)
                               | (INR _ p0) => fun P => (snd X) (fst kt, CValue p0 P)
                               | _ => fun P => False end p).
move => X k k'. case => v P L. simpl. case: v P ; try by [] ;
...
downclosed (fun kt => let: CValue v' p' := snd kt in
 ...
 | S k,FOLD t v => fun X => (R ((s,(fst P))) (snd P)) (k,CValue v X)
 | _,_ => fun X => False
  end p').
(*=End *)
...
Definition xx (h:cheap) (l:nat) : (l \in dom (heap h)) -> (cvalue O).
case => h c l x.
have A:=elimT allP c l x. rewrite findom_indom in x. simpl in x. case: (h l) x A ; last by [].
move => v _ A. by apply (CValue v A).
Defined.

Definition cheap_app (h:cheap) (l:nat) : option (cvalue O) :=
match (bool_option (l \in dom (heap h))) with 
| inl e => Some (xx e)
| inr e => None
end.

Coercion cheap_app : cheap >-> Funclass.

(*=heap_world *)
Definition heap_world k (h:cheap) (w:W) := 
    forall j, j < k -> dom (heap h) =i dom (Unfold w) /\
...
- move => X j L h h' v ev hv. specialize (X j L h h' v ev hv). case: X => w' [LL [HV Y]].
  ...
  specialize (E (k - j, CValue v (proj2 (cev ev)))). rewrite <- E. by apply Y.
- move => X j L h h' v ev hv. specialize (X j L h h' v ev hv). case: X => w' [LL [HV Y]].
  exists w'. split ; first by []. split ; first by []. specialize (E w' (k-j,CValue _ (proj2 (cev ev)))). simpl in E.
...
Definition upred_allt n R s : upred_pcmType (cvalue 0) := mk_upred (@upred_all_down n R s).
...
(*=End *)
   downclosed (fun kt => let: (k,v) := kt in
    match k,(v:cvalue O) with
     ...
     | S k,CValue (LOC l) _ => Unfold w l = k.+1 = Some (R s)
     | _,_ => False
    end).
move => n R s w.
move => m k v L. case: m L ; first by []. move => m L. case: v. case ; try done.
move => l C X. case: k L ; first by []. move => k L. by apply (Mrel_mono (ltnW L) X).
Qed.

(*=upred_reft *)
Definition upred_reft n R w : upred_pcmType (cvalue 0) := mk_upred (@upred_ref_down n R (fst w) (snd w)).
(*=End *)
...
- move => m.
  ...
  case: v. case ; try done. move => l C. have X:=fnonexpansive Unfold e'. clear e'.
  simpl in e. simpl in X. move => L. case: m L e X ; first by []. move => m L e X. simpl.
...
- move => s. move => w w' Lw. simpl. case => k v. simpl. case: k ; first by []. case: v. case ; try done.
  move => l C k. have L:=fmonotonic Unfold Lw. clear Lw. specialize (L l).
...
(*CLEARED*) 
Definition upred_int : upred_pcmType (cvalue 0) := Eval hnf in mk_upred upred_int_down.
(*=End *)
...
Definition upred_unit : upred_pcmType (cvalue 0) := Eval hnf in mk_upred upred_unit_down.
...
Lemma upred_arrow_down n
  ...
  (s:TVal n) (w: halve_pcmType W) : downclosed (fun kt => let: (k,v) := kt in
   ...
   | CValue (LAM t' e) p => forall w' j (va:cvalue O), w <= w' -> (j <= k)%N -> 
              (R0 s w') (j,va) -> IExp (R1 s) j (csubstE [:: va] e) w'
   | _ => False end).
(*=End *)
move => n R0 R1 s w.
...
Lemma IExp_nonexp m (f f':TV) (k:nat) (e:cexpression 0) (w w':halve_cmetricType W) :
   f = m = f' -> w = m = w' -> k < m -> IExp f k e w =-= IExp f' k e w'.
...
- move => X j L h h' v ev hv. specialize (X j L h h' v ev).
  ...
  specialize (E' (k - j) (CValue v (proj2 (cev ev)))).
...
Lemma IStore_down n (Se:StoreType n) (s:TVal n) : 
  downclosed (fun kt => forall l t, Se l = Some t ->
                        (IVal (Ref t) s (snd kt)) (fst kt, cLOC _ l)). (*CLEAR*)
move => n Se s.
...
Definition IStore n (Se:StoreType n) (s:TVal n) : upred_pcmType W :=
    Eval hnf in mk_upred (@IStore_down n Se s).
(*=End *)
...
Definition VRel n (E:TypeEnv n) (Se:StoreType n) (v:Value n) (t:Ty n) := 
  forall k (s:TVal n) (ts:Prod (Ty 0) n) g w,
    (IEnv E s w) (k,g) -> (IStore Se s) (k,w) ->
   (IVal t s w) (k, csubstV (Prod_subst g) (vtsubst (Prod_subst ts) v)).
...
Lemma FT_var n m (E:TypeEnv n) s w k g t : nth_error E m = Some t -> (IEnv E s w) (k, g) ->
     (IVal t s w) (k,nth (cUNIT _) (Prod_subst g) m).
Proof.
...
- move => n t IH s s' k X Y w. rewrite tshiftL_ref. simpl. specialize (IH s s' k).
  ...
  move => l C. specialize (IH X Y). case: (Unfold w l) ; last by [].
  move => a. split => A ; rewrite -> A.
  + by apply: (proj2 (Mrefl _ _) IH _).
  + apply Msym. by apply: (proj2 (Mrefl _ _) IH _).
Qed.

Lemma IEnv_shift n (E:TypeEnv n) s w k x g e : (IEnv E s w) (k, g) ->
    (IEnv (map (@tshiftL 0 1 _) E) (s, x) w) (k, (eq_rect _ (Prod (cvalue 0)) g _ e)).
...
Lemma IEnv_upclosed n E (s:TVal n) k v w w' : w <= w' -> (IEnv E s w) (k,v) -> (IEnv E s w') (k,v).
...
Lemma upred_fun_imp k n (X:upred_pcmType (cvalue n)) (a:Value n) (A B:closedV a) : X (k,CValue _ A) -> X (k,CValue _ B).
...
- move => t IH s m s' a X. simpl. move => w. simpl. case => k. case. case ; try done. move => j C.
  simpl. case: k ; first by []. move => k. specialize (IH s _ s' a X).
  split => A  ; rewrite -> A. by apply: (proj2 (Mrefl _ _) IH _).
  apply Msym. by apply: (proj2 (Mrefl _ _) IH _).
Qed.
...
- move => t' l e e'. rewrite e'. clear e' t. move => k s ts g w. case: k ; first by move => Ig Is. move => k Ig Is.
  simpl. simpl in Is. specialize (Is l t' e). by rewrite -> Is.
...
- move => e t' e'. rewrite e'. clear t e'. move => X Y k s ts g w. move => A B. move => t r w' j Lj. simpl. move => Lw.
  ...
  have C: (IStore (post_compt (@tshiftL 0 1 _) S) (s, r)) (j, w').
    ...
    move => k. simpl. move => A ; rewrite -> A. move => x j v Lj.
    rewrite e' in D. simpl in D. case: D => D. rewrite <- D.
    have B:= @IVal_shift _ Sl s (s,r) O _ _ x (j,v). rewrite <- B ; first by [].
    + clear. simpl. unfold addn. simpl. move => j _ P' P. by rewrite (eq_irrelevance P P').
    + by [].
...
- move => t0 t1 b e. rewrite e. clear t e. move => Db IH. move => k s ts g w Ig Is. simpl.
  ...
  have AA:(IEnv (t0 :: E) s w') (j, (g, va)). split ; last by [].
...
- move => e0 e1 t' _ IH0 _ IH1. move => k s ts g w Ig Is.
  ...
  have A: (IEnv (t' :: E) s w1) (k - n0, (g, CValue _ Cv0)).
  split ; simpl ; last by  apply (upred_fun_imp _ IH0).
    by apply: (IEnv_upclosed Lw) ; apply: (upred_downclosed _ Ig) ; apply leq_subr.
  specialize (IH1 A).
  have B: (IStore S s) (k - n0, w1) by apply (IStore_upclosed Lw) ; apply: (upred_downclosed _ Is) ; apply leq_subr.
  specialize (IH1 B n1).
...
- move => v t' _ IH e. rewrite e. clear e t. move => k s ts g w Ig Is. simpl. move => j Lj h h' v' ev hv.
  ...
  split.
  + move => m Lm. split.
    * move => a. rewrite (proj1 (ufe _)). rewrite <- H1. do 2 rewrite indomUpdMap.
      have aa:=proj1 (hv m (ssrnat.ltn_trans Lm _)). rewrite subSS in aa. rewrite subn0 in aa.
      specialize (aa (ltnSn _)). by rewrite aa.
    * move => l1.
      case_eq (Unfold (Fold (updMap l (IVal t' s) (Unfold w))) l1) ; last by [].
      move => fl0 E'. have E1 :Unfold (Fold (updMap l (IVal t' s) (Unfold w))) l1 =-= Some fl0 by rewrite E'.
      clear E'. rewrite -> (proj2 (ufe _)) in E1. 
      case_eq (l == l1) => e.
      - rewrite <- (eqP e). rewrite <- (eqP e) in E1. clear e l1. rewrite updMap_simpl in E1.
        unfold tset_eq in E1. simpl in E1. clear v0 H.
        have ee:heap h' l = Some (substV (map (@cval _) (Prod_subst g)) (vtsubst (Prod_subst ts) v)).
           rewrite <- H1. by rewrite updMap_simpl.
        case_eq (h' l) ; last by []. move => hl' e.
        have xxx:= (E1 (Fold (updMap l (IVal t' s) (Unfold w)))) (m,hl'). rewrite <- xxx. clear E1 fl0 xxx.
        specialize (IH m s ts g (Fold (updMap l (IVal t' s) (Unfold w)))).
     have eee:Some (csubstV (Prod_subst g) (vtsubst (Prod_subst ts) v)) = Some hl'. rewrite <- e.
         unfold csubstV. clear e hl'. unfold cheap_app. case_eq (bool_option (l \in dom (heap h'))). Focus 2. simpl.
          move => aa. case: aa. rewrite findom_indom. by rewrite ee. Unfocus.
         move => aa. move => _. f_equal.  unfold xx. clear H1. case: h' aa ee. simpl.
         move => h' c aa ee. have bb:=(findom_indom h' l). rewrite (eq_irrelevance (findom_indom h' l) bb).
         have cc:=(allP c l aa). rewrite (eq_irrelevance (allP c l aa) cc).
         move: cc bb ee. case: (h' l). move => hl' cl bb. case => ee. move: cl bb. rewrite ee. clear hl' ee.
         move => cl bb. simpl. unfold Comparison.repack in bb. simpl in bb.
         generalize bb. move: aa. rewrite bb. move => aa. rewrite (eq_irrelevance aa (refl_equal _)). clear aa.
         clear bb. move => bb. rewrite (eq_irrelevance bb (refl_equal _)). clear bb. unfold eq_rec_r. simpl.
         by rewrite -> (eq_irrelevance _ cl). by [].
       case: eee => eee. rewrite <- eee. clear hl' eee e. apply IH.
       move: Ig Lm. clear. elim: E g ; first by [].
       move => t E IH. simpl. case. simpl. move => g v. move => [A B] L.
       specialize (IH g A L). split ; first by apply IH.
       - unfold IEnv.


    * move => l1. rewrite -> (proj2 (ufe _)).
